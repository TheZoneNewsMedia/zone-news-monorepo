...
app.get('/health', async (req, res) => {
  try { await connectDb(); res.json({ status: 'ok', database: 'connected' }); } catch (e) { res.status(500).json({ status: 'error' }); }
});
...
app.get('/api/news', async (req, res) => {
  try {
    ...
    res.json({ success: true, articles: filtered, totalArticles: filtered.length, totalPages: Math.ceil(totalCount / parseInt(limit)), currentPage: parseInt(page), metadata: { scope, category, city: scope === 'local' ? city : null, source: 'Zone News Adelaide Channel' } });
  } catch (e) {
...
app.get('/api/trending', async (req, res) => {
  try {
    ...
    const trending = await db
      ...
      .limit(10)
      .toArray();
    res.json({ success: true, trending: trending.map((a) => ({ id: a._id, title: a.title, views: a.views ?? a.zone_news_data?.views ?? 0, category: categorizeArticle(a) })) });
  } catch (e) { res.status(500).json({ success: false, error: 'Failed to fetch trending articles' }); }
});
...
app.get('/api/breaking', async (req, res) => {
  try {
    ...
    const breaking = await db
      .collection('news_articles')
      .find({ $and: [{ $or: [{ 'zone_news_data.channel': '@ZoneNewsAdl' }, { source: { $regex: 'Zone News', $options: 'i' } }] }, { published_date: { $gte: since } }] })
      .sort({ published_date: -1 })
      .limit(5)
      .toArray();
    res.json({ success: true, breaking: breaking.map((a) => ({ id: a._id, title: a.title, content: a.content ? `${a.content.substring(0, 150)}...` : '', publishedAt: a.published_date, category: categorizeArticle(a) })) });
  } catch (e) { res.status(500).json({ success: false, error: 'Failed to fetch breaking news' }); }
});
...
app.get('/api/stats', async (req, res) => {
  try {
    ...
    const categories = Object.keys(categoryMap).reduce((acc, c) => { acc[c] = articles.filter((a) => categorizeArticle(a) === c).length; return acc; }, {});
    res.json({ success: true, stats: { totalArticles, todayArticles, categories } });
  } catch (e) { res.status(500).json({ success: false, error: 'Failed to fetch statistics' }); }
});

app.listen(PORT, '0.0.0.0', () => console.log(`news-api listening on ${PORT}`));
